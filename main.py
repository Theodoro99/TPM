...
def main(page: ft.Page):
    ...
    def initialize_views(page):
        ...
        with SessionLocal() as session:
            ...
            open_count = session.query(func.count(LogbookEntry.id)).filter(LogbookEntry.status == StatusEnum.OPEN).scalar() or 0
            completed_count = session.query(func.count(LogbookEntry.id)).filter(LogbookEntry.status == StatusEnum.COMPLETED).scalar() or 0
            escalated_count = session.query(func.count(LogbookEntry.id)).filter(LogbookEntry.status == StatusEnum.ESCALATION).scalar() or 0
...
PORT = int(os.getenv("PORT", "8558"))  # Changed to 8558 to avoid port conflicts
